...
import {isSupportedFileFormat} from "./isSupportedFileFormat.ts";
...
function fromMarkdown(this: any, opts: FromMarkdownOptions = {}) {
    ...
    function enterWikiLink(token:any) {
        this.enter(
            {
                type: "wikiLink",
                data: {
    ...
    function exitWikiLinkTarget(token:any) {
        const target = this.sliceSerialize(token);
        const current = top(this.stack);
        current.data.target = target;
    }

    function exitWikiLinkAlias(token:any) {
